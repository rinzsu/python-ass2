"""
this program helps the user to manage reminders by loading data from csv files, displaying active reminders, 
and allowing the user to interact with them via a command-line interface

the features this reminder system has are:
- viewing active reminders
- adding new reminders (now or later)
...
- a unique identifier (reminder_id)
- a reminder text (e.g: "buy milk")
- an activation time: stored in 'reminders_active_database'
- a dismissal time: stored in 'reminders_dismissed_database' if the reminder has been dismissed
...
the program allows users to add, update, close, and renew reminders. 
data is stored in a csv file (can be read and modified via the cli) 
actions are logged, and repeating reminder text is avoided, even for recurring reminders

the program loads reminders from CSV files and shows all active ones in a numbered list 
users can manage them using simple commands

the 'dump to <FILENAME>' command saves all reminders (active, future, and past) to the file specified by the user
each reminder includes its reminder_id, reminder_text, active_from, and dismissed_at values
if a reminder has not been dismissed, its dismissed_at value will be set to 'datetime.fromtimestamp(0)'
"""
...
if "__main__" == __name__:
    ...
    while (user_input != "quit"):
        ...
        # renew a previously dismissed reminder or scheduled one
            ...
            # split user input to get the reminder ID and new activation datetime
            renew_id_text, renew_datetime=  user_input.split(" at ")
            _, renew_id = renew_id_text.split()
            
            # get past, active, and future reminders
            past_reminders = data.get_past_reminders()
            active_reminders = data.get_active_reminders()
            future_reminders = data.get_future_reminders()

            # initialize variable for the renewed reminder ID
            renewed_id = 0

            # handle reminder ID renewal for past or future reminders
            if (int(renew_id) < 0):
            ...
            # renew reminder with new activation datetime
            if (renewed_id != 0):
            ...
            # display updated active reminders
            print("ACTIVE REMINDERS")
            active_reminders = data.get_active_reminders()
            data.print_reminders_by_index(active_reminders)

        # dump current state to a csv file
        elif ("dump to" in user_input):
            _, filename=  user_input.split(" to ")
            data.dump_database(filename)
